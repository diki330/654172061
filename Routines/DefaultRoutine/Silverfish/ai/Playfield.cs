    ...
    public class Playfield
    {
        ...
        public void doAction(Action aa)
        {
            ...
            if (a.actionType == actionEnum.attackWithMinion)
            {
                ...
                //secret stuff
                bool hasCounterSecret = false;
                int newTarget = 0;
                if (this.enemySecretCount > 0)
                {
                    foreach (Handmanager.Handcard hc in this.owncards)
                    {
                        if ((hc.card.name == CardDB.cardName.eaterofsecrets && (this.ownMaxMana - this.lockedMana >= 4)) ||
                           (hc.card.name == CardDB.cardName.kezanmystic && (this.ownMaxMana - this.lockedMana >= 4)))
                        {
                            hasCounterSecret = true;
                        }
                    }
                    if (!hasCounterSecret)
                    {
                        newTarget = this.secretTrigger_CharIsAttacked(a.own, target);
                    }
                    else
                    {
                        newTarget = 0;
        ...
        public void minionTransform(Minion m, CardDB.Card c)
        {
            ...
            if (m.name == CardDB.cardName.madscientist)
            {
                if (!m.own)
                    this.evaluatePenality -= 5;
            }
