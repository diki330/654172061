namespace HREngine.Bots
{
    ...
    public class PenalityManager
    {
        ...
        public int getAttackWithMininonPenality(Minion m, Playfield p, Minion target)//随从攻击惩罚
        {
            ...
            if (!p.isLethalCheck && m.name == CardDB.cardName.bloodimp) retval += 50;//鲜血小鬼除非斩杀不打人
            switch (m.name)
            {
                case CardDB.cardName.leeroyjenkins://火车王不打雏龙
                    if (!target.own && target.name == CardDB.cardName.whelp) return 500;
                    break;
                case CardDB.cardName.bloodmagethalnos://血法师
                ...
                //case CardDB.cardName.flamewreathedfaceless:
            ...
            switch (name)
            {
                ...
                case CardDB.cardName.splittingaxe:
                    ...
                    else if (TotemCount > 0 && TotemCount <= 3)
                    {
                        ...
                        {
                            return 48 - ownTotemsValue * 8;
                        }
                        else if (ownTotemsValue >= 4 && ownTotemsValue < 8) //一个灼热一个嘲讽到一个治疗一个法强
                        {
                            return 32 - ownTotemsValue * 4;
                        }
                        else if (ownTotemsValue >= 8) //一个治疗一个法强以上，就打出去
                        {
            ...
            switch (card.name)
            {
                ...
                case CardDB.cardName.eaterofsecrets:
                    if (p.enemySecretCount > 0)
                    {
                        ...
                        if (p.playactions.Count == 0) pen -= 30;
                    }
                    else
                    {
                        switch (p.enemyHeroStartClass)
                        {
                            case TAG_CLASS.MAGE: pen += 50; break;
                            case TAG_CLASS.PALADIN: pen += 5; break;
                            case TAG_CLASS.HUNTER: pen += 5; break;
                        }
                    }
        ...
        private int getPlayCardSecretPenality(CardDB.Card c, Playfield p)
        {
            ...
            /*if (p.enemyHeroStartClass == TAG_CLASS.MAGE)
            {
                if (c.type == CardDB.cardtype.MOB)
                {
                    ...
                    // play first the small minion:关闭防镜像实体
        ...
        private int getAttackSecretPenality(Minion m, Playfield p, Minion target)
        {
            ...
            if (p.enemyHeroStartClass == TAG_CLASS.MAGE)
            {
                ...
                bool ishigh = isOwnHighestFlameWard(m, p);
                if (attackedbefore == 0 && ishigh) pen -= 20;
                if (attackedbefore == 0 && !ishigh) pen += 10;
            ...
            /*if (p.enemyHeroStartClass == TAG_CLASS.MAGE)
            {
                if (target.isHero)
                {
                    ...
                    foreach (SecretItem si in p.enemySecretList)
                    {
                        if (si.canBe_flameward) { canBe_flameward = false; break; }
                    }
                    if (canBe_flameward)
                    {
                        if (!target.own)
        ...
        private bool isOwnHighestFlameWard(Minion mnn, Playfield p)
        {
            if (mnn.Hp >= 4)
            {
                return false;
        ...
        private void setupEnrageDatabase()//激怒数据库
        {
        ...
        private void setupHealDatabase()//治疗数据库
        {
        ...
        private void setupDamageDatabase()//伤害数据库
        {
        ...
        private void setupPriorityList()//随从优先级
        {
            ...
            priorityDatabase.Add(CardDB.cardName.sorcerersapprentice, 5);
        ...
        private void setupUsefulNeedKeepDatabase()//保护随从优先级数据库
        {
        ...
        private void setupDiscardCards()//弃牌数据库
        {
        ...
        private void setupSpecialMins()//特殊随从数据库
        {
        ...
        private void setupEnemyTargetPriority()//敌方随从优先级
        {
        ...
        private void setupSilenceTargets()//沉默目标
        {
