        ...
        public bool didAttackOrderMatters(Playfield p)
        {
            ...
            foreach (Minion m in ownm)
            {
                ...
                if (m.Ready && m.Angr >= 1)
                {
                    ...
                    if (m.name == CardDB.cardName.raidleader || m.name == CardDB.cardName.stormwindchampion || m.name == CardDB.cardName.timberwolf || m.name == CardDB.cardName.southseacaptain || m.name == CardDB.cardName.murlocwarleader || m.name == CardDB.cardName.grimscaleoracle || m.name == CardDB.cardName.leokk || m.name == CardDB.cardName.fallenhero || m.name == CardDB.cardName.warhorsetrainer || m.name == CardDB.cardName.vessina) return true;
